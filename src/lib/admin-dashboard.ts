...
export async function computeAdminDashboardSnapshot(): Promise<AdminDashboardSnapshot> {
  ...
  // ── 2. Coverage breadth ───────────────────────────────────────────
  const insurerCounts = countBy(
    parsedPolicies,
    (p) => (p.insurerName ?? "").trim() || "(unknown)"
  );
  const topInsurers = topN(insurerCounts, 10);

  const mmCounts = countBy(
    parsedPolicies,
    (p) => `${(p.vehicle?.make ?? "").trim()} ${(p.vehicle?.model ?? "").trim()}`.trim()
  );
  const topMakeModels = topN(mmCounts, 10);

  const rtoCounts = countBy(
    parsedPolicies,
    (p) => (p.vehicle?.rto ?? "").trim() || "(unknown)"
  );
  const topRtos = topN(rtoCounts, 10).map(({ name, count }) => ({
    rto: name,
    count,
  }));
  ...
  // Gap categories from report.keyGaps.items[].title
  ...
  const topGapCategories = topN(gapCounts, 10).map(({ name, count }) => ({
    category: name,
    count,
  }));
  ...
  // ── 5. Channel mix ────────────────────────────────────────────────
  // Heuristic: uploadedPdfUrl path contains "/inbox/" → email forward.
  let webUpload = 0;
  let emailForward = 0;
  for (const p of parsedPolicies) {
    const url = p.uploadedPdfUrl ?? "";
  ...
  // within 30-min window). Count clusters with 2+ docs.
  const forwardsByOwner = new Map<string, ParsedPolicy[]>();
  for (const p of parsedPolicies) {
    const url = p.uploadedPdfUrl ?? "";
    if (!(url.includes("/inbox/") || url.includes("inbox/"))) continue;
  ...
  return {
    ...
    breadth: {
      insurerCount: insurerCounts.size,
      topInsurers,
      makeModelCount: mmCounts.size,
      topMakeModels,
      rtoCount: rtoCounts.size,
      topRtos,
      oldestExpiry,
      newestExpiry,
      policyCount,
